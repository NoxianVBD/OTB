...
function [precision, fps] = run_tracker(video, show_visualization, show_plots)
	...
	base_path = 'Z:\lct-tracker-master\Benchmark';
	%default settings
...
%     config.appearance_thresh=0.38;
	...
	otherwise
        ...
        %[img_files, pos, target_sz,newgroundtruth_rect, video_path] = load_video_info(base_path, video);
        %test
        rect_h = getrect ;
		fid = fopen('Z:\lct-tracker-master\Benchmark\bird2\groundtruth_rect.txt', 'w');
        fprintf(fid, '%.f',rect_h(1));
         for i = 2:4
             fprintf(fid, ',%.f',rect_h(i));
         end
         fclose(fid);
        %test
        ...
        % precisions = precision_plot(positions, newgroundtruth_rect, video, show_plots);
