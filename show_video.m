...
%         im=imresize(im,1/resize_image);
		...
		%render target bounding box for this frame
         ...
         %rect_h =  rectangle('Position',[0,0,1,1], 'EdgeColor','r', 'Parent',axes_h);
         % rect_new = rectangle('Position',[0,0,1,1], 'EdgeColor','r', 'Parent',axes_h)
        ...
        %rect_h1 =  rectangle('Position',getrect, 'EdgeColor','r', 'Parent',axes_h);
        %end 
        %my
       ...
       % if ~isempty(boxes{frame}),
		%	set(rect_new, 'Visible', 'on', 'Position', boxes{frame});
            
		%else
		%	set(rect_new, 'Visible', 'off');
